...
class LatestLinFit(OptimiserStrategy):
    """Get the next variable value by lin fitting the data but when the length
    of the data points exceeds 10 only the last 10 points are considered. This
    makes a linear fit from those latest data points and then gets the variable
    value that results in an intersection with the desired_output_param_value.
    This is identical to the LinFit OptimiserStrategy appart from this
    exclusion of early data points.
    ...
    def get_next_variable_param_value(
        ...
        x_data = np.array(variable_param_values)[-10:]
        y_data = np.array(desired_output_param_values)[-10:]
